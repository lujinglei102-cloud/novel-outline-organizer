// 纯 Node 单元测试（0 依赖，可直接 node test/engine.test.ts）
...
function transpileTS(code: string): string {
  ...
  // 去掉 TS 函数/变量类型标注 : X 或 : X | Y | Z 或 : X[] 或 : { a:T, b:U }
  {
    ...
    function readTypeExpr(s: string, start: number): number {
      let j = start
      // 跳过前导空白
    ...
    type Frame = { q: number, obj: boolean }
    let out = ''
    let i = 0
    const stack: Frame[] = [{ q: 0, obj: false }]
    let top = 0
    function prevNonSpace(p: number): string {
      p--
      while (p >= 0) {
...
function rewriteImports(js: string, sourceFile: string): string {
  const fixLocal = (m: string, pre: string, p: string, post: string) => {
...
function ensureDir(d: string) { try { mkdirSync(d, { recursive: true }) } catch {} }
ensureDir(outDir)

// 收集并编译所有 engine + data + types(skip) + utils/export 模块
const modulesToCompile: string[] = [
...
import('node:test').then(async ({ describe, it }) => {
  const passed: string[] = []
  const failed: { name: string; err: Error }[] = []
  async function check(name: string, fn: () => void | Promise<void>) {
    try { await fn(); passed.push(name); console.log('  ✔', name) }
    catch (e) { failed.push({ name, err: e as Error }); console.log('  ✘', name, '\n   ', (e as Error).message) }
  }
  ...
  // ===== 数据 =====
  function mkCard(id: string, content: string, overrides: any = {}): any {
    return {
      id,
      content,
  ...
  await check('能按频率排序角色', () => {
    ...
    assert.equal(r.characters.find((c: any) => c.name === '沈知行')?.name, '沈知行')
  })
  await check('角色有代表性描述', () => {
    const cards = [mkCard('1', '沈知行第一次见到林婉清，她正在雨中哭')]
    const r = ce.extractCharacters(cards)
    const shen = r.characters.find((c: any) => c.name === '沈知行')
  ...
  await check('正负情绪同时存在时生成冲突标签', () => {
    ...
    const r = ce.extractCharacters(cards)
    const shen = r.characters.find((c: any) => c.name === '林婉清')
    if (shen) {
  ...
  await check('拆章节时能给每个段分章节', () => {
    ...
    const cardIdsAll = new Set<string>()
